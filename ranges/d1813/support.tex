...
\begin{itemdescr}
   \pnum
   Specialisation of \tcode{left_identity}'s first parameter is permitted, but specialising the
...
\begin{itemdescr}
   \pnum
   Specialisation of \tcode{right_identity}'s first parameter is permitted, but specialising the
   latter two parameters is ill-formed, no diagnostic required. All specialisations must take the
   form:
...
\begin{itemdescr}
   \pnum
   Let \tcode{left1} be an object of type \tcode{left_identity<BOp, T, U>}, \tcode{left2} be an
   object of type \tcode{left_identity<BOp, U, T>}, \tcode{right1} be an object of type
   ...
   \pnum
   If \tcode{t != left1()} is \tcode{true} and \tcode{u != right1()} is
   \tcode{true}, then the expressions \tcode{t == invoke(bop, t, right())} and
   \tcode{u == invoke(bop, left(), u)} are both \tcode{true}.
\end{itemdescr}
...
\begin{itemdescr}
   \pnum
   Specialisation of \tcode{left_zero}'s first parameter is permitted, but specialising the
   latter two parameters is ill-formed, no diagnostic required. All specialisations must take the
   form:
...
\begin{itemdescr}
   \pnum
   Specialisation of \tcode{right_zero}'s first parameter is permitted, but specialising the
   latter two parameters is ill-formed, no diagnostic required. All specialisations must take the
   form:
...
\begin{itemdescr}
   \pnum
   Let \tcode{left1} be an object of type \tcode{left_zero<BOp, T, U>}, \tcode{left2} be an
   object of type \tcode{left_zero<BOp, U, T>}, \tcode{right1} be an object of type
   ...
   \pnum
   If \tcode{t != left1()} is \tcode{true} and \tcode{u != right1()} is
   \tcode{true}, then the expressions \tcode{right1() == invoke(bop, t, right1())} and
...
\reviewernote{
   ...
   been co-opted by category theory. An less attractive alternative to \tcode{magma} is
   \tcode{closed_operation}, but the author does not recommend this name (it probably conveys as
   much information to non-mathematicians as \tcode{magma}).
}
...
\reviewernote{
   ...
   aren't required by an algorithm, but given their `simple' definition, it would be remiss to omit
...
\end{itemdecl}
\reviewernote{
   This is not the same as the dreaded \tcode{has_plus}; it's more of a \tcode{has_plus+}. There
   should also probably be some axioms, but the author hasn't yet figured out how to describe them.
   (All other arithmetic function objects have explicit axioms.)
}
...
\begin{itemdescr}
   \pnum
   Let \tcode{t} be an object of type \tcode{T}.

   \pnum
   The expression \tcode{-t != t} is \tcode{true}.
...
\reviewernote{
   ...
   \tcode{inverse_operation<negate>} specialisation where \tcode{operator()} returns
}
...
\reviewernote{
   ...
   would like to take the opportunity of renmaing this function object so that one can more
   naturally describe the computation.\\
   \\
   A potential alternative is \tcode{product}, this is the result of multiplication, not the
   operation itself (we'd need to renmae \tcode{plus} to \tcode{sum}, etc., to facilitate that
   idea).
}
...
\begin{itemdescr}
   ...
   \pnum
   The expressions \tcode{t * T\{\} == T\{\}}, \tcode{u * U\{\} == U\{\}},
   \tcode{t * U\{\} == U\{\}}, and \tcode{u * T\{\} == T\{\}} are all \tcode{true}.

   \pnum
   The expressions \tcode{t * T\{1\} == t}, \tcode{T\{1\} * t == t}, \tcode{u * U\{1\} == u},
   \tcode{U\{1\} * u == u}, \tcode{u * T\{1\} == u}, \tcode{T\{1\} * u == u},
   \tcode{t * U\{1\} == t}, and \tcode{U\{1\} * t == t} are all \tcode{true}.

   \pnum
   TODO: state that \tcode{t1 * t2} is equivalent to a summation of itself in a way that doesn't
   unnecessarily restrict matrices and other mathematical objects.
\end{itemdescr}
...
\begin{itemdecl}
  ...
  struct left_identity<times, T, U> {
    constexpr explicit left_identity(const times&, const T&, const U&)
    {}
  ...
  template<class T, class U>
  requires magma<times, T, U>
  struct right_identity<times, T, U> {
    constexpr explicit right_identity(const times&, const T&, const U&)
    {}
  ...
  template<class T, class U>
  requires magma<times, T, U>
  struct left_zero<times, T, U> {
    constexpr explicit left_zero(const times&, const T&, const U&)
    {}
  ...
  template<class T, class U>
  requires magma<times, T, U>
  struct right_zero<times, T, U> {
    constexpr explicit right_zero(const times&, const T&, const U&)
    {}
...
\begin{itemdescr}
   ...
   \tcode{T\{\} / u2 == common_type_t<T, U>\{\}}, \tcode{U\{\} / t2 == common_type_t<T, U>\{\}} are
   all \tcode{true}.

   \pnum
   TODO: insert requirement that specifies division is repeated subtraction.
\end{itemdescr}
...
\begin{itemdecl}
  ...
  struct right_identity<divided_by, T, U> {
    constexpr explicit right_identity(divided_by, const T&, const U&)
    {}
...
\begin{itemdecl}
  ...
  struct left_zero<modulus, T, U> {
    constexpr explicit left_zero(modulus, const T&, const U&)
    {}
