...
like. Below is a rough approximation of what `SequenceContainer` might look like in pseudo-Java.

```java
interface Iterator<T> {}
interface ConstIterator<T> {}

interface SequenceContainer<T> {
   Iterator<T> begin();
   Iterator<T> end();
   ConstIterator<T> cbegin();
   ConstIterator<T> cend();

   boolean isEqual(SequenceContainer<T> other);

   void swap(SequenceContainer<T> other);
   SequenceContainer<T> assign(SequenceContainer<T> other);

   long size();
   long max_size();
   boolean empty();

   void insert(T t) throws OutOfMemoryError;
   void clear();
}

public class vector<T> implements SequenceContainer<T> {
   public vector()  { /* insert implementation here */ }
   public vector(vector<T> a)  { /* insert implementation here */ }

   private class iterator implements Iterator<T> {}
   public Iterator<T> begin()  { return new iterator(); }
   public Iterator<T> end()  { return new iterator(); }

   private class const_iterator implements ConstIterator<T> {}
   public ConstIterator<T> cbegin()  { return new const_iterator(); }
   public ConstIterator<T> cend()  { return new const_iterator(); }

   public boolean isEqual(SequenceContainer<T> other) { return true; }
   public void swap(SequenceContainer<T> other) { /* insert implementation here */ }
   public SequenceContainer<T> assign(SequenceContainer<T> other) { return this; }

   public long size() { return 0; }
   public long max_size() { return 0; }
   public boolean empty() { return true; }

   public void insert(T t) throws OutOfMemoryError { /* insert implementation here */ }
   public void clear() { /* insert implementation here */ }
}
```

For those unfamiliar with Java, an `interface` prescribes a type's interface, dictating the minimum
number of member functions that a class conforming to the interface must provide; otherwise it is
an abstract class. The closest that C++ has to offer here are virtual functions.

Although the primary purpose of concepts is not to mimic Java's `interface`, this behaviour in C++
is likely inevitable, and the spelling for `class vector<T> implements SequenceContainer<T>` in C++
is `static_assert(SequenceContainer<vector<T>>);` inside a class definition, not
`void clear(SequenceContainer auto& c);`. Expressing requirements on type parameters through
concepts is to describe the _minimal_ interface needed for well-formed usage. This is why the
range-based `std::ranges::find` requires an `InputRange`, not a `Container`. As such, the author
deems that the `clear` function provided does not fall within the design-space of concept usage, and
is not a valid counter-example.
